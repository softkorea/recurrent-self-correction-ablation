...
class RecurrentMLP:
    ...
    def forward_sequence_vn(self, x_seq, T=3):
        """Variable-noise T-step unroll. 각 timestep에서 다른 입력 사용.

        Static-input tautology를 해소: x_t = prototype + ε_t (독립 노이즈).

        Args:
            x_seq: (T, input_size) array — 각 timestep별 입력
            T: 타임스텝 수 (기본 3)
