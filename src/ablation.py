...
def forward_sequence_interpolated(target_net, x, alpha, interp_type='zero',
    ...
    """Interpolated feedback: feedback = α·y_self + (1-α)·y_other.
...
def forward_sequence_with_clone_vn(target_net, clone_net, x_seq, T=3):
    """VN clone feedback: 매 timestep 다른 입력 + clone 피드백.
...
def fit_learned_affine(target_net, donor_net, X_calib, T=3):
    """Calibration set에서 donor→target logit 선형 회귀 학습.

    양 모델을 self-feedback로 정상 실행하여 (donor_output, target_output) 쌍을 수집하고,
    target ≈ donor @ W + b 를 최소제곱법으로 피팅.
...
def fit_learned_affine_vn(target_net, donor_net, X_seq_calib, T=3):
    """VN calibration: donor→target logit 선형 회귀 학습.
...
def forward_sequence_with_learned_affine_clone(target_net, donor_net, x,
    ...
    donor는 self-feedback로 정상 실행, donor의 출력을
    W_align @ donor_output + b_align 변환 후 target에 주입.
