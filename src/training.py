...
제약:
- forward_sequence 시 샘플마다 reset_state()
- 3-step unroll backprop. 수치미분은 gradient check 전용. rel error < 1e-4.
- BPTT 에러 2회 이상 시 수치미분으로 전면 대체.
...
def generate_data_variable_noise(n_samples, noise_level, T=3, n_classes=5,
    ...
    """Variable-noise 데이터 생성: 매 timestep 독립 노이즈.

    x_t = prototype_k + ε_t, where ε_t ~ N(0, σ²I), ε_1 ⊥ ε_2 ⊥ ε_3.
    Static-input tautology를 해소하면서 memory-correction 분리를 유지.
...
def compute_loss_and_gradients_vn(net, x_seq, target, T=3, time_weights=None):
    """VN 단일 샘플 loss + gradients. 매 timestep 다른 입력.

    backward pass는 static 버전과 동일 — cache['x']가 이미 per-timestep 입력을 저장.
    ...
    Args:
        ...
        T: timestep 수
        time_weights: 시간 가중 (기본 [0.0, 0.2, 1.0])
    """
    if time_weights is None:
        time_weights = [0.0, 0.2, 1.0]
...
def gradient_check_vn(net, x_seq, target, T=3, epsilon=1e-5):
    """VN analytical vs numerical gradient 비교. max relative error 반환."""
