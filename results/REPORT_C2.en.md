...
Worse than even C1 (-0.064). Self-correction depends on **the model's own output trajectory**,
not just any reasonable feedback signal.
...
1. **C2 ≈ C1 (or worse)**: Across all noise levels, C2's gain is comparable to or lower than C1
...
### 4.3 Why C2 Is Worse Than C1

Interestingly, C2 tends to be slightly worse than C1:
- C1's shuffled feedback is meaningless, so the network may partially "ignore" it
- C2's clone output is **meaningful but wrong**, causing the network to actively follow it into larger errors
